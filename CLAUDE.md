...
| Vercel env vars | ❌ Missing — checkout is dead in production | Verified in a browser: `https://www.noelleandmary.com/checkout` shows "Online checkout isn't available yet" — no Square SDK, no card iframe. The `NEXT_PUBLIC_SQUARE_*` values are absent at build time. `NGF_ORDERS_SECRET` is almost certainly absent too, which would fail order reporting independently of Square. Content is unaffected. All 11 vars + a redeploy needed, since `NEXT_PUBLIC_*` are baked in at build. |
