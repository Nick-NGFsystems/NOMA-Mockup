...
| `lib/site-data.ts` | Hardcoded fallback data for products, bundles, reviews. The product list is the BUILT-IN catalogue, used only until NOMA's products exist in the portal |
| `lib/ngf-products.ts` | The catalogue: NOMA's products and prices from the portal's Products page (`GET /api/public/products`), for display AND checkout. Falls back to the built-in list while the portal has none; checkout refuses if the portal cannot be read |
...
| Products from the portal | ⚠️ Built 2026-09-25, needs NGF-Systems-app PR #25 deployed | Shop page, home best sellers, checkout page and `/api/checkout` all read `lib/ngf-products.ts`. Until products are entered on NOMA's Products page the site sells the built-in list exactly as before, editor product fields included. Once they are, the portal list replaces it, best sellers are the products marked "Feature on the home page" (else the first six), the Metal filter matches each product's tags, and the product cards stop carrying editor annotations, so products and prices are edited only on the Products page. Entering them retires the "One necklace, two names" row below: the name is whatever NOMA types. Checked locally against a mock portal in all three states (none, products, unreadable); never against the live app. Bundles stay editor-managed display cards (not purchasable). |
